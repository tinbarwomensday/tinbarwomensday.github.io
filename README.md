...
$$\sum\limits_{x=start}^{end} (some_equation)$$

Product:

$$\prod\limits_{x=start}^{end} (some_equation)$$
...
returning a number from 0 to -1 if A <= B, 0 if A >= B. But we want the result to be 0 or 1, so we can just add 1 then divide by 2, now we have:
